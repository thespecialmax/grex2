...
$(document).ready(function() {
	...
	$('.do-search').on('click', function() {
		...
		var menu_type = $('.search-field input[name=\'search\']').closest('.search-field').find('.menu_type').val();
		var value = '';
		if ($("body").hasClass("product-search")) {
			$('.search-field input[name=\'search\']').each(function () {
				value = $(this).val() ;
			});
		}else{
			value = $('.search-field input[name=\'search\']').val();
		}
	...
	$('.main-search-input').on('keydown', function(e) {
		if (e.keyCode == 13) {
			...
			var menu_type = $('.search-field input[name=\'search\']').closest('.search-field').find('.menu_type').val();
			if (filter_name) {url += '&search=' + encodeURIComponent(filter_name);}
			if (menu_type) {url += '&menu_type=' + encodeURIComponent(menu_type);}
			var category_id = $('select[name=\'category_id\']').prop('value');
			if (category_id > 0) {
				url += '&category_id=' + encodeURIComponent(category_id) + '&sub_category=true';
		...
		$.ajax({
			...
			$('[data-toggle=\'tooltip\']').tooltip('hide');
				...
				if (json['success_redirect']) {
					...
					$('.common-mini-cart').load('index.php?route=common/cart/info .common-mini-cart > *');
				}
			},
